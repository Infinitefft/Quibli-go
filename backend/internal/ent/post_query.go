...
type PostQuery struct {
	...
	withLikes     *UserLikePostQuery
	withFavorites *UserFavoritePostQuery
	withFKs       bool
...
func (pq *PostQuery) QueryTags() *PostTagQuery {
...
func (pq *PostQuery) QueryLikes() *UserLikePostQuery {
	query := (&UserLikePostClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PostQuery) QueryFavorites() *UserFavoritePostQuery {
	query := (&UserFavoritePostClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PostQuery) WithTags(opts ...func(*PostTagQuery)) *PostQuery {
	query := (&PostTagClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *PostQuery) WithLikes(opts ...func(*UserLikePostQuery)) *PostQuery {
	query := (&UserLikePostClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *PostQuery) WithFavorites(opts ...func(*UserFavoritePostQuery)) *PostQuery {
	query := (&UserFavoritePostClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *PostQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Post, error) {
	...
	if query := pq.withLikes; query != nil {
		if err := pq.loadLikes(ctx, query, nodes,
			func(n *Post) { n.Edges.Likes = []*UserLikePost{} },
			func(n *Post, e *UserLikePost) { n.Edges.Likes = append(n.Edges.Likes, e) }); err != nil {
			return nil, err
		}
	}
	if query := pq.withFavorites; query != nil {
		if err := pq.loadFavorites(ctx, query, nodes,
			func(n *Post) { n.Edges.Favorites = []*UserFavoritePost{} },
			func(n *Post, e *UserFavoritePost) { n.Edges.Favorites = append(n.Edges.Favorites, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PostQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Post, init func(*Post), assign func(*Post, *User)) error {
	...
	query.Where(predicate.User(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(post.UserColumn), fks...))
	}))
...
func (pq *PostQuery) loadTags(ctx context.Context, query *PostTagQuery, nodes []*Post, init func(*Post), assign func(*Post, *PostTag)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Post)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
...
func (pq *PostQuery) loadLikes(ctx context.Context, query *UserLikePostQuery, nodes []*Post, init func(*Post), assign func(*Post, *UserLikePost)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Post)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
...
func (pq *PostQuery) loadFavorites(ctx context.Context, query *UserFavoritePostQuery, nodes []*Post, init func(*Post), assign func(*Post, *UserFavoritePost)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Post)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	for _, n := range neighbors {
		fk := n.post_favorites
		if fk == nil {
			return fmt.Errorf(`foreign-key "post_favorites" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "post_favorites" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
