...
type TagQuery struct {
	...
	withPosts     *PostTagQuery
	withQuestions *QuestionTagQuery
	withFKs       bool
...
func (tq *TagQuery) QueryPosts() *PostTagQuery {
	query := (&PostTagClient{config: tq.config}).Query()
...
func (tq *TagQuery) QueryQuestions() *QuestionTagQuery {
...
func (tq *TagQuery) WithPosts(opts ...func(*PostTagQuery)) *TagQuery {
	query := (&PostTagClient{config: tq.config}).Query()
...
func (tq *TagQuery) WithQuestions(opts ...func(*QuestionTagQuery)) *TagQuery {
	query := (&QuestionTagClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := tq.withPosts; query != nil {
		if err := tq.loadPosts(ctx, query, nodes,
			func(n *Tag) { n.Edges.Posts = []*PostTag{} },
			func(n *Tag, e *PostTag) { n.Edges.Posts = append(n.Edges.Posts, e) }); err != nil {
			return nil, err
		}
	}
	if query := tq.withQuestions; query != nil {
		if err := tq.loadQuestions(ctx, query, nodes,
			func(n *Tag) { n.Edges.Questions = []*QuestionTag{} },
			func(n *Tag, e *QuestionTag) { n.Edges.Questions = append(n.Edges.Questions, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TagQuery) loadPosts(ctx context.Context, query *PostTagQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *PostTag)) error {
	...
	query.Where(predicate.PostTag(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(tag.PostsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.tag_posts
		if fk == nil {
			return fmt.Errorf(`foreign-key "tag_posts" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "tag_posts" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (tq *TagQuery) loadQuestions(ctx context.Context, query *QuestionTagQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *QuestionTag)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Tag)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.QuestionTag(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(tag.QuestionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.tag_questions
		if fk == nil {
			return fmt.Errorf(`foreign-key "tag_questions" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "tag_questions" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
